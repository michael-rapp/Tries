...
/**
 * Defines the interface, a class, which defines the structure of a trie, must implement. This
 * interface provides methods, which are invoked when inserting, removing or retrieving keys from a
 * trie. By implementing these methods accordingly, the structure of the trie can be adjusted.
 *
...
public interface Structure<SequenceType extends Sequence, ValueType> {
     ...
     * corresponds to a specific sequence. Depending on the trie's structure, the suffix of the
     * sequence can be processed to any extend.
     *
    ...
    /**
     * The method, which is invoked in order to add a successor to a specific node. Depending on the
